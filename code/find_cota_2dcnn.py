...
img_ext = "2dcnn_%s_120122"%h5_ext
...
# Fit data to model
history = model.fit([pix_train], [cota_train],
...
print("cota training time for 2dcnn",time.clock()-train_time_x)

start = time.clock()
cota_pred = model.predict([pix_test], batch_size=9000)
...
plot_residuals(residuals_cota,'2dcnn','cota',img_ext)
