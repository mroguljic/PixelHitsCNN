...
x = Activation("relu")(x)
x = Dense(1)(x)
cotb = Activation("linear", name="cotb")(x)
...
# Fit data to model
history = model.fit([pix_train], [cotb_train],
...
start = time.clock()
cotb_pred = model.predict([pix_test], batch_size=9000)
...
plot_residuals(residuals_cotb,'2dcnn','cotb',img_ext)
