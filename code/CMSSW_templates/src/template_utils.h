...
struct FrontEndModel
{
    ...
    //--- PhaseIII - initial guess
    double thresholdph3 = 600; // threshold in e-
    double qperToTph3 = 900; // e- per TOT
    ...
    float apply_model( float qin, float ygauss, float zgauss )
    {
        ...
        switch ( fe_type ) { 
            ...
            case 4: //dual slope ToT (current phase 3)
                {
                float qin_new = qin + zgauss * readout_noise;
                if(qin_new < thresholdph3) return 0;
                int kinkpoint = int(ADCMax/2) + 1;
                int adc = std::floor((qin_new - thresholdph3)/qperToTph3);
                if (adc >= kinkpoint){
                    qin_new = qin + zgauss*readout_noise * dualslope; //scale noise to be larger
                    adc = std::floor((qin_new - thresholdph3)/qperToTph3);
                    adc = floor((adc - kinkpoint)/dualslope) + kinkpoint;
                }
                adc = std::min(adc, ADCMax);
                ...
                if(adc < kinkpoint){
                    signal = int((adc + 0.5)* qperToTph3 + thresholdph3);
                }
                else{
                    signal = int((adc + 0.5 - kinkpoint) * dualslope*qperToTph3 + (kinkpoint * qperToTph3) + thresholdph3);
                }
                //printf("In %.0f adc %i out %.0f \n", qin, adc, signal);
