...
class Infer2DCNN_single : public edm::one::EDAnalyzer<> {
public:
	explicit Infer2DCNN_single(const edm::ParameterSet&);
	~Infer2DCNN_single(){};
...
/*
std::unique_ptr<CacheData> Infer2DCNN_single::initializeGlobalCache(const edm::ParameterSet& config) 
{
	...
	graphPath_ = config.getParameter<std::string>("graphPath_");
...
void Infer2DCNN_single::globalEndJob(const CacheData* cacheData) {
	// reset the graphDefi
...
*/
void Infer2DCNN_single::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
Infer2DCNN_single::Infer2DCNN_single(const edm::ParameterSet& config)
...
void Infer2DCNN_single::beginJob() {
  // load the graph
...
void Infer2DCNN_single::analyze(const edm::Event& event, const edm::EventSetup& setup) {
...
//	printf("count = %i\n",count);
	...
	DEFINE_FWK_MODULE(Infer2DCNN_single);
