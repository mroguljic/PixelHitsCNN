...
class InferDNN_x : public edm::stream::EDAnalyzer<edm::GlobalCache<CacheData>> {
public:
	explicit InferDNN_x(const edm::ParameterSet&, const CacheData*);
	~InferDNN_x(){};
	...
	std::unique_ptr<CacheData> InferDNN_x::initializeGlobalCache(const edm::ParameterSet& config) 
	{
	...
	void InferDNN_x::globalEndJob(const CacheData* cacheData) {
	// reset the graphDef
	...
	void InferDNN_x::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
	...
	InferDNN_x::InferDNN_x(const edm::ParameterSet& config, const CacheData* cacheData)
...
//fTrackCollectionLabel(config.getUntrackedParameter<InputTag>("trackCollectionLabel", edm::InputTag("ALCARECOTkAlMuonIsolated"))),
	...
	//		cnn_file = fopen(infile2, "w");
			...
			sprintf(infile3,"%s/dnn_MC_x.txt",path);
			cnn_file = fopen(infile3, "w");
		
	}

	void InferDNN_x::beginJob() {
	/*
	printf("IN BEGINJOB");
 ...
 // fTree->Branch("x_gen",        x_gen,       "x_gen");
	...
	*/
	...
	void InferDNN_x::analyze(const edm::Event& event, const edm::EventSetup& setup) {
...
//			printf("fails after filling buffer\n");
		...
		//	printf("fails after lp\n");
				...
				tensorflow::Tensor cluster_flat_x(tensorflow::DT_FLOAT, {1,TXSIZE});
    		// angles
