...
class InferDNN_y : public edm::stream::EDAnalyzer<edm::GlobalCache<CacheData>> {
public:
	explicit InferDNN_y(const edm::ParameterSet&, const CacheData*);
	~InferDNN_y(){};
	...
	std::unique_ptr<CacheData> InferDNN_y::initializeGlobalCache(const edm::ParameterSet& config) 
	{
	...
	void InferDNN_y::globalEndJob(const CacheData* cacheData) {
	// reset the graphDef
	...
	void InferDNN_y::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
	...
	InferDNN_y::InferDNN_y(const edm::ParameterSet& config, const CacheData* cacheData)
...
//fTrackCollectionLabel(config.getUntrackedParameter<InputTag>("trackCollectionLabel", edm::InputTag("ALCARECOTkAlMuonIsolated"))),
 ...
 // fTree->Branch("y_gen",        y_gen,       "y_gen");
	...
	*/
	...
	void InferDNN_y::analyze(const edm::Event& event, const edm::EventSetup& setup) {
